...
import { getDocById, updateDocument } from '@/db/operations';
import { useLocalSearchParams, router } from 'expo-router';

export default function EditFirm() {
    const params = useLocalSearchParams();
    const firmId = String(params.id || '');
    ...
    useEffect(() => {
        const load = async () => {
            try {
                const data = await getDocById('insuranceFirms', firmId);
                if (data) {
                ...
                ToastAndroid.show('Failed to load firm', ToastAndroid.SHORT);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await updateDocument('insuranceFirms', firmId, {
            ...
            ToastAndroid.show('Firm updated', ToastAndroid.SHORT);
            router.back();
        } catch (e) {
    ...
    return (
        <View style={[styles.container, { backgroundColor: background }]}>
            <Heading page='Edit Insurance Firm' />
            <ScrollView style={styles.content}>
                ...
                <TouchableOpacity onPress={handleSave} disabled={saving} activeOpacity={0.7} style={[styles.submitButton, { backgroundColor: accent }]}>
                    {saving ? <ActivityIndicator color={'white'} /> : <Text style={styles.submitText}>Save Changes</Text>}
                </TouchableOpacity>
            </ScrollView>
        </View>
    );
