...
selectedCargoArea: TruckTypeProps | null; // Adjust 'any' to actual image type if known
  ...
  dspPersonalTrucks: boolean;
  user: any; // Define a more specific type for user
  trucks: any[]; // Define a more specific type for trucks
  refreshing: boolean;
  onRefresh: () => void;
  loadMoreTrucks: () => void;
  ...
  bg: string; // Assuming 'bg' is passed as a prop for background color
  icon: string; // Assuming 'icon' is passed as a prop for icon color
  accent: string; // Assuming 'accent' is passed as a prop for accent color
  background: string; // Assuming 'background' is passed as a prop for general background color
}
    ...
    <View style={{ flex: 1}}>
      ...
      <View style={[styles.container, { backgroundColor: bg }]}>
        ...
        >
          ...
          {dspPersonalTrucks && (
            <View>
              <ThemedText>{user?.organisation} </ThemedText>
            </View>
          )}
        ...
        <FlatList
          ...
          data={trucks}
          renderItem={({ item }) => <TruckItemComponent truck={item} />}
          refreshControl={
